...
class User(db.Model):
    ...
    def to_dict(self):
        return dict(studentid=self.studentid, firstname=self.firstname, lastname=self.lastname, email=self.email)
...
def borrow_book():
    ...
    required_fields = {'studentid':str , 'bookid': str}

    if not data or not set(required_fields).issubset(data.keys()):
